    ...
    <section className="bg-midnight">
        ...
        <Heading>Our Menu</Heading>
            <div className="md:flex md:gap-8 pt-3">
                ...
                <div className="flex-1">
                    {spaMenu.map(({ title, services }) => (
                        <div key={title} className="mb-12">
                            <h2 className="text-2xl font-semibold mb-6 border-y-2 py-3">{title}</h2>
                            <div className="space-y-6">
                                {services.map(({ packages, price }, index) => (
                                    <div
                                    key={index}
                                    className="flex sm:flex-row items-center justify-between pb-3"
                                    >
                                        <span className="text-lg font-medium">{packages}</span>
                                        <div className="block flex-grow border-t mx-4 border-black" />
                ...
                {/* Right column */}
                <div className="flex-1">
                    {spaOptions.map(({ title, services }) => (
                        <div key={title} className="pb-10 lg:pl-6 lg:pb-14 lg:border-l-2">
                            <h2 className="text-2xl font-semibold mb-6 border-y-2 py-3">{title}</h2>
                            <div className="space-y-6">
                                {services.map(({ packages, price }, index) => (
                                    <div key={index}
